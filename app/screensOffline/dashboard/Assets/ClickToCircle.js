         ...
         />
          ...
          {string && Questions[currentIndex].image!=null ?
                   <Image
                      ...
                      resizeMode={'contain'}
                      source={{uri:'file://'+Questions[currentIndex].image}}
                  ...
                  renderItem={({item, index}) =>(
                    <Box style={{padding:10}}>
                        <TouchableOpacity onPress={()=>setCheckAnswer(item)}>
                                <Badge rounded={'lg'} size={50} style={{borderRadius:70,width:70,height:70}} colorScheme={selectedOption==item?"success":"warning"}  variant={(selectedOption==item && wrongAnswer==false) ||(selectedWrongOption==item && wrongAnswer)?'solid':'outline'}>
