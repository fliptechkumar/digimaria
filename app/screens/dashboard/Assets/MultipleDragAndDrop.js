...
export default function DragandDrop(props) {
	...
	const [questions, setQuestions] = useState([]);
		...
		useEffect(() => {
			if(props?.route?.params?.assets?.questions && props?.route?.params?.assets?.questions.length>0 && props?.route?.params?.assets?.questions[0]?.question){
		 ...
		 // https://digimaria.com/Chapter_Images/Class2/Mathpath/chapter_1/monkey.png
		 //https://digimaria.com/Chapter_Images/Class2/Mathpath/chapter_1/monkey.png
					...
					<DraxProvider>
						<View style={styles.container}>
							<View style={styles.receivingContainer}>
								{
									questions && questions.length>0 && questions[currentIndex]?.question.map((item, index) => ReceivingZoneUIComponent(item, index, questions[currentIndex].answer[index]))
								}
							</View>

							<View style={styles.draxListContainer}>
								{questions && questions.length>0 && questions[currentIndex]?.options.map((item, index) => {
						...
						<View style={{ flexDirection: 'row', padding: 5 }}>
							...
							<View style={{ flex: 1, justifyContent: 'center' }}>
								<Text
									...
									{currentIndex + 1}/{questions && questions.length}
								</Text>
							</View>
