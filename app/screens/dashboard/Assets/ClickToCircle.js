...
// ]
  ...
  const setCheckAnswer = (value,index) => {
   // setShow(false);
    ...
    console.log(index,Questions[currentIndex].answer)
  ...
  return (
    <Box flex="1" >
      <HeaderComponent
        ...
        LeftContent={'goback'}
      />
      {props?.route?.params?.assets?.chapter?
       <View style={{padding:10}}>
          <Text style={{fontSize:16,color:'#0AB4B6',fontWeight:'bold'}}>{props?.route?.params?.assets?.chapter}</Text>
       </View>:false}     
       <View style={{
            shadowColor: '#808080',
            backgroundColor:'#fff',
            ...
            numColumns={2}
                  ...
                  renderItem={({item, index}) =>(
                    <Box style={{padding:10}}>
                        <TouchableOpacity onPress={()=>setCheckAnswer(item,index+1)}>
                                <Badge rounded={'2xl'} size={50} style={{borderRadius:70,width:70,height:70}} colorScheme={selectedOption==item?"success":"warning"}  variant={(selectedOption==item && wrongAnswer==false) ||(selectedWrongOption==item && wrongAnswer)?'solid':'outline'}>
                                      <Text style={{fontSize:16,padding:5,color:selectedOption==item?"#fff":'orange',fontWeight:'bold'}}>{item}</Text>
                                </Badge>
                        </TouchableOpacity>
                   </Box>
                  )}
