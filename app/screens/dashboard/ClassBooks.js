...
export default function Home(props) {
  ...
  const {getBooks,getSubjectContent,setContentType,getChapters, chapterList,setLocalSubjectContent,ResetData,subjectContent} = useDetailsAll();
 useEffect(() => {
  ...
  const LinkBook = async (userId) => {
		setLoading(true);
		let resource = 'classsubjects?';
		let filter='userid='+userId+'&classid='+props.route.params.classId
...
useEffect(() => {
  ...
  const OpenTypeBased = item => {
    console.log('itemhh',item)
    setId(item);
    setSelectedSubject(item);
    ResetData();
    if(item.type=='individual'){
        if(item.ebook==null || item.ebook==''){
        props.navigation.navigate('Chapters', { id: item.id, name: item.subject_name });
        }else{
  ...
  return (
    <Box flex="1" style={{backgroundColor: '#eaebed'}}>
      <HeaderComponent
        wrapperColor="#151218"
        bgColor="#fece2e"
        headerTitle={props.route.params.classname}
        nav={props.navigation}
        LeftContent={'goback'}
       // subTitle={props.route.params && props.route.params.title}
      />
      {loading ? <Loader /> : false}
      {internetError && <InternetError data={internetError} />}
      <View style={{flex: 1, margin: 10}}>
        {props.route.params && props.route.params.title?
        <Text style={{fontSize: 18, fontWeight: 'bold', color: 'green'}}> {props.route.params.title}</Text>:null}
        ...
        <Actionsheet isOpen={showTermOrSem} onClose={onCancelSub}>
          <Actionsheet.Content style={{backgroundColor: '#f9bdb8'}}>
              ...
              <View style={{width: '100%'}}>
                ...
                <View style={{width: '100%', marginTop: 10}}>
                {selectedSubject && selectedSubject.ebook ? (
                  <TouchableOpacity
                    onPress={() => OpenModel('ebook', id)}
                    style={[
                      {
