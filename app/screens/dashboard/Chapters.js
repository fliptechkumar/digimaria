...
export default function Home(props) {
	...
	const { getChapters, chapterList, contentsList,setLocalChapterList,loading } = useDetailsAll();
	const DestinationRoot = RNFetchBlob.fs.dirs.DocumentDir + '/' + 'MyApp/images';
	const iosAsset = RNFetchBlob.fs.dirs.DownloadDir + '/' + 'MyApp/images'
	useEffect(() => {
		//alert(iosAsset)
	  ...
	  const ImageExist =  (data,type) => {
		data.map((item,index)=>{
			for(let i=0;i<item.questionsRow.length;i++){
				...
				var pathUrl = item.questionsRow[i];
...
const getChaptersLocally = async (subjectid) => {
	...
	const DownLoadVideo =  (url, id) => {
		...
		//inside ios/hazzel/assets folder destination
		const destinationPath =  iosAsset;
...
const DeleteTask =  (id) => {
	db.transaction(function (txn) {
		txn.executeSql('DELETE FROM Tasks WHERE chapterid = '+id, [], function (tx, res) {
			console.log('res', res)
			checkTaskLocally(id);
		})
	})
			...
			<View style={{ margin: 10, flex: 1 }}>
			       <Box  style={[
						...
						<View style={{flexDirection:'row',flex:1}}>
							<Text style={{ fontSize: 18, fontWeight: 'bold', color: '#fff',flex:3}}>{props.route.params.name}</Text>
							<View  style={{flex:0.5 }}>
			...
			<Actionsheet isOpen={showContent} onClose={onCancel}>
				<Actionsheet.Content style={{ backgroundColor: '#f1e8c7' }}>
					...
					{selectedChapter && selectedChapter.video ? (
                        ...
                        !showDownload? <MyIcon name="download" size={20} color="#fff" />:
                       ...
                       flex:3
                          ...
                          {TaskDownloaded ? (
                          <TouchableOpacity onPress={()=>DeleteTask(selectedChapter.id)} style={{flex:1}}>
                            ...
                            !showDownload? 
							<TouchableOpacity style={{flex:1}} onPress={() => OpenModel('chapters', id)}>
									<View style={{alignItems:'flex-end'}}>
						...
						<TouchableOpacity onPress={() => { 
							...
							<View style={{ justifyContent: 'center' , flexDirection: 'row',justifyContent:'space-between' }}>
								...
								TaskDownloaded ? (
											<MyIcon name="list-alt" size={20} color="#fff" />
										) : (
											<MyIcon name="download" size={20} color="#fff" />
										) )}
